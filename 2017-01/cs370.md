...
If you have a sequence of $$N$$ points $$(x_i, y_i)$$ and you want to connect them *in the order that they appear in the sequence*, introduce a "time" parameter $$t_i$$ such that $$x(t_i) = x_i$$ and $$y(t_i) = y_i$$ and interpolate $$x(t)$$ and $$y(t)$$ separately.

Two ways to pick $$t$$:

- Simple: $$t_i = i$$
- Approximate arc-length parameterization: $$t_0 = 0$$, $$t_{i+1} = t_i + \sqrt{\Delta x^2 + \Delta y^2}$$

## Ordinary differential equations

We are given $$ y'(t) = f(t, y(t)) $$ and $$ y(t_0) = y_0 $$. This may not have a closed form. How do we approximate the solution?

There are several types of ways.
- Explicit vs. implicit: explicit lets you plug and chug, implicit requires you to solve stuff.
- Single-step vs. Multistep: number of points the algorithm looks back

### Forward Euler

$$
y_{n+1} = y_n + hy_n ^\prime
$$

- Integral interpretation: Rectangles, top-left corner matched.
- Local truncation error is $$ O(h^2) $$.

### Trapezoidal scheme

Derived from the first few terms of the Taylor series.

$$
y_{n+1} = y_n + \frac{h}{2} [ f(t_{n+1}, y_{n+1}) + f(t_n, y_n) ]
$$

- Requires solving a system to obtain $$ y_{n+1} $$.
- Integral interpretation: Trapezoids.
- Local truncation error is $$ O(h^3) $$.

### Improved Euler

This is an obtained from the trapezoidal rule obtained by using $$ y(t_{n+1}) \approx y^*_{n+1} $$.

$$
\begin{eqnarray}
y^*_{n+1}  & = & y(t_n) + hf(t_n, y(t_n)) \\
y(t_{n+1}) & = & y(t_n) + \frac{h}{2} [f(t_n, y(t_n)) + f(t_n+1, y^*_{n+1})]
$$
...
This is a family of methods like Forward Euler and Imrpoved Euler where stuff is based off of previous stuff. For example, RK4 is like parabolas instead of trapezoids.

### Backwards Euler

- Single-step implicit

### Adams-Bashforth

- A fancy explicit multistep with LTE $$O(h^3)$$.

### Global error

The number of steps is $$ O(h^{-1}) $$, so the global error is the local error one order down.

### Stability

Note: Stability is a separate concept from accuracy. To test the stability of a time-stepping scheme, plug in The Test Equation

$$
y'(t) = -\lambda y(t), \lambda > 0
$$

Then solve for the closed form and see for what values it "blows up" relative to the initial value.
